"""
Glue PySpark ETL script to upload edges table to the RDS.
...
class LoadArtistsETL:
    ...
    def transform(self, dyf):
        """
        Extracts distinct edges from the given DynamicFrame.
        ...
        edge_records = ApplyMapping.apply(dyf, [
            ("playlist_id", "number", "playlist_id", "string"),
            ("track_id", "string", "track_id", "string"),
        ])

        edges_df = edge_records.toDF().drop_duplicates(["playlist_id", "track_id"])

        return DynamicFrame.fromDF(edges_df, self.glue_context, OUTPUT_RDS_TABLE_NAME)
