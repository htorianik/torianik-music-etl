"""
Glue PySpark ETL script to upload artists table to the RDS.
...
class LoadArtistsETL:
    ...
    def transform(self, dyf):
        """
        Extracts distinct artists from the given DynamicFrame.
        ...
        artist_records = ApplyMapping.apply(dyf, [
            ("artist_id", "string", "id", "string"),
            ("artist_name", "string", "name", "string")
        ])

        artists_df = artist_records.toDF().drop_duplicates(["id"])

        return DynamicFrame.fromDF(artists_df, self.glue_context, OUTPUT_RDS_TABLE_NAME)
